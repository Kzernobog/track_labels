...
class YOLODetector(object):
	#constructor
	def __init__(self, config_path="tiny-tank-yolo-V3.cfg", weight_path="tiny-tank-yolo-V3_38400.weights", meta_path="tank.data", debug=False ):
		...
		for detection in detections: 
			...
			if draw:
				if (confidence*100) > 55:
					cv2.rectangle(img, (int(xCoord), int(yCoord)), (int(xCoord + xEntent), int(yCoord + yExtent)), [255,255,255], 2)
				else:
					cv2.rectangle(img, (int(xCoord), int(yCoord)), (int(xCoord + xEntent), int(yCoord + yExtent)), [255,255,255], 2)
			self.frame_detection_ratio(frame_dimensions,detected_list,imcaption)
			if self.debug_flag:
				...
				cv2.putText(img,pstring,(boundingBox[0][0],boundingBox[0][1]),cv2.FONT_HERSHEY_PLAIN,1,(255,255,255),2,cv2.LINE_AA)
				cv2.putText(img,str(current_ratio_with_frame),(boundingBox[0][0],boundingBox[0][1]-20),cv2.FONT_HERSHEY_PLAIN,1,(255,255,255),2,cv2.LINE_AA)
				pass
		# if draw:
...
if __name__=='__main__':
		yolo=YOLODetector()
		img=cv2.cv2.imread(r"C:\Users\Legend\Documents\TankApp_Dataset\Desert_tank_data(T80)\TankImg29.png")
